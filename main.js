...
var DEFAULT_IMAGE_MODEL = "gpt-image-1";
var DEFAULT_SPEECH_MODEL = "whisper-1";
var DEFAULT_API_BASE_URL = "https://api.openai.com/v1";
var DEFAULT_MAX_TOKENS = 0;

// src/openai_api.ts
var normalizeBaseUrl = (baseUrl) => {
  var _a2;
  const trimmed = (_a2 = baseUrl == null ? void 0 : baseUrl.trim()) != null ? _a2 : "";
  const normalized = trimmed || DEFAULT_API_BASE_URL;
  return normalized.replace(/\/?$/, "");
};
var OpenAIAssistant = class {
  constructor(apiKey, baseUrl, modelName, maxTokens, speechModelName) {
    ...
    this.text_api_call = async (prompt_list, htmlEl, view) => {
      ...
      const sendRequest = async (omitMaxTokens2) => {
        var _a2, _b, _c, _d, _e, _f;
        ...
        if (includeMaxTokens && !omitMaxTokens2) {
        ...
        return (_f = (_e = (_d = response.choices) == null ? void 0 : _d[0]) == null ? void 0 : _e.message) == null ? void 0 : _f.content;
      };
    ...
    this.img_api_call = async (model, prompt, img_size, num_img, is_hd) => {
      ...
      try {
        ...
        const data = (_a2 = response == null ? void 0 : response.data) != null ? _a2 : [];
  ...
  shouldRetryWithoutMaxTokens(err) {
    const parts = [];
    if (err instanceof OpenAI.APIError) {
      if (err.message) {
...
var AnthropicAssistant = class {
  constructor(apiKey, baseUrl, modelName, maxTokens) {
    this.display_error = (err) => {
      this.showNotice(err);
    };
    this.text_api_call = async (prompt_list, htmlEl, view) => {
      ...
      try {
        ...
        return (_c = (_b = (_a2 = parsed.content) == null ? void 0 : _a2[0]) == null ? void 0 : _b.text) != null ? _c : "";
      } catch (err) {
        this.display_error(err);
      }
    };
    this.img_api_call = async () => {
      this.showNotice(
    ...
    this.whisper_api_call = async () => {
      this.showNotice(
        "Speech-to-text requires an OpenAI-compatible endpoint."
      );
    };
    this.text_to_speech_call = async () => {
...
var AiAssistantPlugin = class extends import_obsidian3.Plugin {
  ...
  buildAssistants() {
    const slotCache = /* @__PURE__ */ new Map();
  ...
  createAssistant(apiKey, baseUrl) {
  ...
  getProviderConfig(slot) {
    const keyField = `apiKey${slot}`;
    const urlField = `apiBaseUrl${slot}`;
  ...
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        if (!assistant) {
          new import_obsidian3.Notice("Configure a provider for chat in mini assistant settings.");
          return;
        }
    ...
    this.addCommand({
      ...
      editorCallback: (editor) => {
        ...
        if (!assistant) {
          new import_obsidian3.Notice("Configure a provider for speech-to-text in mini assistant settings.");
          return;
        }
        new SpeechModal(
          this.app,
  ...
  migrateSettings(data) {
    ...
    migrated.apiKey1 = ((_a2 = migrated.apiKey1) != null ? _a2 : "").toString().trim();
    migrated.apiKey2 = ((_b = migrated.apiKey2) != null ? _b : "").toString().trim();
    const legacyBase = typeof data.apiBaseUrl === "string" && data.apiBaseUrl.trim() ? data.apiBaseUrl.trim() : DEFAULT_API_BASE_URL;
    if (typeof migrated.apiBaseUrl1 !== "string") {
      migrated.apiBaseUrl1 = legacyBase;
    }
    migrated.apiBaseUrl1 = ((_c = migrated.apiBaseUrl1) != null ? _c : "").toString().trim();
    migrated.apiBaseUrl2 = ((_d = migrated.apiBaseUrl2) != null ? _d : "").toString().trim();
    ...
    const parsedTokens = Number.parseInt(
      String((_f = (_e = data == null ? void 0 : data.maxTokens) != null ? _e : migrated.maxTokens) != null ? _f : ""),
      10
    );
    ...
    delete migrated.apiKey3;
    delete migrated.apiBaseUrl3;
    delete migrated.imageProvider;
    delete migrated.imageModelName;
    delete migrated.imgFolder;
    delete migrated.mySetting;
    return migrated;
  }
};
var AiAssistantSettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Chat Model").setDesc("Type the exact model identifier from your provider").addText((text) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Max Tokens").addText((text) => {
      text.setPlaceholder("Leave blank to use provider default");
      text.setValue(
        this.plugin.settings.maxTokens > 0 ? this.plugin.settings.maxTokens.toString() : ""
      );
      text.onChange(async (value) => {
        ...
        if (Number.isNaN(intValue) || intValue < 0) {
          new import_obsidian3.Notice("Max tokens must be a positive number or blank.");
          return;
        }
    ...
    new import_obsidian3.Setting(containerEl).setName("Prompt behavior").setDesc("Replace selection").addToggle((toggle) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("Speech-to-Text Model").setDesc("Type the transcription model identifier (e.g. whisper-1)").addText((text) => {
    ...
    new import_obsidian3.Setting(containerEl).setName("The language of the input audio").addText((text) => {
  ...
  renderProviderSlot(containerEl, slot) {
    var _a2, _b;
    ...
    const currentKey = (_a2 = this.plugin.settings[keyField]) != null ? _a2 : "";
    new import_obsidian3.Setting(providerDiv).setName("API Key").addText((text) => {
    ...
    const currentBase = (_b = this.plugin.settings[urlField]) != null ? _b : "";
    new import_obsidian3.Setting(providerDiv).setName("Base URL").addText((text) => {
  ...
  renderProviderPicker(containerEl, label, field, desc) {
    const providerOptions = {
      "1": "Provider 1",
      "2": "Provider 2"
    };
    new import_obsidian3.Setting(containerEl).setName(label).setDesc(desc).addDropdown((dropdown) => {
      var _a2;
      dropdown.addOptions(providerOptions);
      dropdown.setValue(String((_a2 = this.plugin.settings[field]) != null ? _a2 : 1));
      dropdown.onChange(async (value) => {
        this.plugin.settings[field] = Number.parseInt(value, 10);
        await this.plugin.saveSettings();
        this.plugin.buildAssistants();
      });
    });
